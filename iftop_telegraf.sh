...
# Get a list of talkers by sampling for 2 seconds, up to 100 streams, and then dump themp out
# to an awk script that pull out all but the hosts, then parse them in a simple python script that outputs the data in telegraf line format.
